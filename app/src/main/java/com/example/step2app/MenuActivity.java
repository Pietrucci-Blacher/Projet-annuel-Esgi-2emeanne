...
public class MenuActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String idDelivery=prefs.getString("idLivraison",null);
        ...
        if(idDelivery.equals("none")){
            deliveryBtn.setText("Effectuer une livraison");
        }else{
            deliveryBtn.setText("Reprendre la livraison");
        }
                        ...
                        alertDialog.setItems(listHour, new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                RequestBody formBody = new FormBody.Builder()
                                        .add("idDelivery", idDelivery)
                                        .add("deposit", idDeposit)
                                        .add("zone", zoneGeo)
                                        .add("time", String.valueOf(hours))
                                        .add("poids", ptac).build();
                                ...
                                MenuActivity.this.runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        try {
                                            if(finalJsonObj.getInt("nbColis") > 0){
                                                toast.cancel();
                                                Intent delFlow = new Intent(MenuActivity.this, DeliveryFlow.class);
                                                delFlow.putExtra("delivery",finalJsonObj.toString());
                                                delFlow.putExtra("idLivraison",idDelivery);
                                                startActivity(delFlow);
                                                finish();
                                            }else{
                                                toast.cancel();
                                                ...
                                                Integer nbParcel = 0;
                                                try {
                                                    nbParcel = Integer.parseInt(finalJsonObj.getString("countReturn"));
                                                } catch (JSONException e) {
                                                    e.printStackTrace();
                                                }
                                                for (int i = 0; i < nbParcel; i++) {
                                                    try {
                                                        returnParcel.add("Référence : "+finalJsonObj.getJSONArray("returnParcel").getJSONObject(i).getString("refQrcode"));
                                                        Log.d("parcel",finalJsonObj.getJSONArray("returnParcel").getJSONObject(i).getString("refQrcode"));
                                                    } catch (JSONException e) {
                                                        e.printStackTrace();
                                                    }
                                                }
                                                Intent returnInt = new Intent(MenuActivity.this, ReturnParcel.class);
                                                returnInt.putStringArrayListExtra("returnParcel", (ArrayList<String>) returnParcel);
                                                returnInt.putExtra("idDelivery", idDelivery);
                                                returnInt.putExtra("nbKm", finalJsonObj.getString("distance"));
